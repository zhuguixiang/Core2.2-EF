...
namespace Core.EF.Application.Commom.Core
{
    ...
    public class EFMapper
    {
        ...
        private static void SetValues(object sourceObject, object targetObject, string[] withProperties, string[] withoutProperties, bool skipVirtual)
        {
            ...
            EFMapperTypeDescription sourceObjectTypeCache = EFMapperCache.Get(sourceObjectType);
            EFMapperTypeDescription targetObjectCache = EFMapperCache.Get(targetObjectType);

            foreach (EFMapperPropertyDescription sourceProperty in sourceObjectTypeCache.PropertyList)
            {
